...
Сценарий: постраничная загрузка пользователей соответствующих части email
	Пусть существуют пользователи
	| имя       | email            |
...
Сценарий: постраничная загрузка пользователей при пустом тексте email
	Пусть существуют пользователи
	| имя       | email            |
	| Медведев  | Сидоров@kreml.uc |
	| Медведева | Петров@kreml.uc  |
	...
	| Кабаева   | Кабаева@kreml.uc |
	| Сабчак    | Сабчак@mail.ru   |
	Когда загружают 1 страницу пользоватлей по 3 пользователя с фильтром по email " "
	Тогда возвращается страница пользователей
	| всего | на странице |
	| 8     | 3           |

Сценарий: постраничная загрузка пользователей соответствующих части имени
	Пусть существуют пользователи
	| имя       |
	| Медведев  |
	| Медведева |
